    ...
    <style type="text/css">
			...
			*
			...
			.line
			{
				...
				width:90%;
				margin-left:5%;
				border-bottom: 1px solid #0E1E3F;
				border-left: 1px solid #0E1E3F;
				border-right: 1px solid #0E1E3F;
			}
...
<form action="include/bn_submit.switch.php" id="submit_form" method="post" target="submit_form_frame">
    ...
    <div id="page_1">
        <div>
    		<img src="images/activity/1011_reg_top.jpg"/>
    	</div>
		<p style="text-align:center;padding-top:10px;font-size:20px;font-weight:bold;background-color:#EFEEEA;overflow:hidden;">
			<span style="font-size:18px;">北欧旅游局</span><br/>
			厦门、大连、南京路演 网上报名启动
		</p>
		<div style="background-color:#EFEEEA;padding-top:5px;padding-bottom:10px;">
		<div style="margin-left:8%;margin-right:8%;width:auto;color:#6b6b6b;font-size:14px;line-height:25px;text-align:justify;text-justify:inter-ideograph;">
			斯堪的纳维亚旅游局（北欧旅游局）携手北欧航空公司、海达路德游轮公司、DFDS游轮公司及荷兰皇家航空公司，将于2016年7月29日厦门、8月3日大连、8月9日南京举办专场推介会，诚邀当地旅行社进行网上报名。
		</div>
		</div>
		<div class="item">
			<div class="box" onclick="change_location(1011)">
				<div class="box1">
					<div id="1011"></div>
				</div>
				&nbsp;&nbsp;厦门
			</div>
			<div class="box" onclick="change_location(1012)">
				<div class="box1">
					<div id="1012"></div>
				</div>
				&nbsp;&nbsp;大连
			</div>
			<div class="box" onclick="change_location(1013)">
				<div class="box1">
					<div id="1013"></div>
				</div>
				&nbsp;&nbsp;南京
    	...
    	<div class="input" style="margin-top:22px;padding-bottom:25px;margin-left:5%;width:90%;">
    	   <button type="button" onclick="submit_info()">立即提交</button>
    	</div>
	</div>
</form>
<script type="text/javascript">
	...
	<?php 
	$a_act1=new WX_Activity(1011);
	$a_act2=new WX_Activity(1012);
	$a_act3=new WX_Activity(1013);
	?>
	var local = {
		    1011:'<?php echo($a_act1->getLocation().'站：'.$a_act1->getActivityDate().'（周'.$a_act1->getWeek().'）');?>',
		    1012:'<?php echo($a_act2->getLocation().'站：'.$a_act2->getActivityDate().'（周'.$a_act2->getWeek().'）');?>',
		    1013:'<?php echo($a_act3->getLocation().'站：'.$a_act3->getActivityDate().'（周'.$a_act3->getWeek().'）');?>',
	}
	<?php 
	echo('
		change_location('.$sceneId.')
	');
	...
	function change_location(id)
	{
		...
		document.getElementById("1012").className="";
		document.getElementById("1013").className="";
		if (local[id]!=undefined)
		{
			...
			document.getElementById('Vcl_Id').value='';
		}		
	}
...
<div class="weui_dialog_alert" id="success" style="display:none;">
    <div class="weui_mask" style="background:rgba(31,49,78,.9)"></div>
    <div class="weui_dialog_loading">
