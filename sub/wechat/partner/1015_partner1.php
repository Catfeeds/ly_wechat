...
<html lang="zh-CN">
<head>
    <title>有奖问答-第一题 </title>
	<meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=0, minimum-scale=1.0, maximum-scale=1.0"/>
	<meta name="description" id="metaDescription" content="有奖问答-第一题">
...
<form action="../include/bn_submit.switch.php" id="submit_form" method="post" target="submit_form_frame">
	...
	<input type="hidden" name="Vcl_Question" id="Vcl_Question" value="1"/>
	...
	<div class="question" style="background-color:white;">
		...
		<div class="item">
		槟城有许多历史悠久，文化底蕴深厚的建筑可作为团队活动的特色场地，以下不在槟城的是：
		</div>
	</div>
	<div class="question">
		<div id="A" class="select" onclick="selected(this,'A')" style="margin-top:40px;">A. 康华利斯堡</div>
		<div id="B" class="select" onclick="selected(this,'B')">B. 龙山堂邱公司</div>
		<div id="C" class="select" onclick="selected(this,'C')">C. The Glasshouse</div>
		<div id="D" class="select" onclick="selected(this,'D')">D. 大卫布朗餐厅</div>
...
if ($o_user->getAllCount()>0)
{
	...
	if ($o_activity->getAllCount()>0)
	{
		...
		{
			if ($o_user_activity_join->getAnswer1(0)!='0')
			{
				//说明已经答过
				?>
				...
				$('#<?php echo($o_user_activity_join->getAnswer1(0))?>').addClass("on error");
