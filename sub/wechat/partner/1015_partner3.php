...
<form action="../include/bn_submit.switch.php" id="submit_form" method="post" target="submit_form_frame">
	...
	<div class="question">
		<div id="A" class="select" onclick="selected(this,'A')" style="margin-top:40px;">A. 世界最高的钢索攀岩</div>
		<div id="B" class="select" onclick="selected(this,'B')">B. 两岛之间世界最长的空中滑索</div>
		<div id="C" class="select" onclick="selected(this,'C')">C. 皇家雪兰莪锡镴制作体验</div>
		<div id="D" class="select" onclick="selected(this,'D')">D. 世界最大的天然山洞徒步</div>
...
if ($o_user->getAllCount()>0)
{
	...
	if ($o_activity->getAllCount()>0)
	{
		...
		{
			if ($o_user_activity_join->getAnswer3(0)!='0')
			{
				//说明已经答过
				?>
				...
				$('#<?php echo($o_user_activity_join->getAnswer3(0))?>').addClass("on error");
