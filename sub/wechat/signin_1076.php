...
if($o_user_activity->getSigninFlag(0)==1)
{
	...
	{
		echo "<script>location.href='signin_success_reg_".$_GET['sceneid'].".php?id=".$sceneId."'</script>"; 
		exit(0);
	}else{
...
<script type="text/javascript">
	...
	function submit_success(id)
	{
		var url=document.getElementById("Vcl_Url").value
		//判断是否加注册成功页面
		if (document.getElementById("Vcl_Reg").value=='1')
		{
			location=url+'signin_success_<?php echo $_GET['sceneid'];?>.php?id='+id;			
		}else{
			location=url+'signin_success_reg_<?php echo $_GET['sceneid'];?>.php?id='+id;
		}
