...
$o_activity->PushWhere(array("&&", "Id", "=",'1074'));
...
<body>
<img class="bj" src="images/1074_bj.jpg" style="width:100%"/>
<div class="body">
	<div class="userphoto">
		<?php echo($s_photo)?>
	</div>
...
function set_resize()
{
	...
	//获取头像顶部距离
	var img_top=$('.userphoto').css('top')
	img_top=parseInt(img_top)
	//获取头像的高度
	var img_height=parseInt($('.userphoto').css('width'))
	//计算姓名顶部距离
	var name_top=img_top+img_height+15

	$('.username').css('top',name_top+'px')
	//计算按钮顶部距离
	var button_top=img_top+img_height+90
	$('.button').css('top',button_top+'px')

	//计算头像距离左侧距离
	var img_left=parseInt($('.userphoto').css('margin-left'))
	var img_width=img_height
	//定位姓名左侧距离
	$('.username').css('margin-left',Math.round(img_left+img_width/2-150)+'px')
	//定位按钮左侧距离
	$('.button').css('margin-left',Math.round(img_left+img_width/2-114)+'px')
}
var Round =[];
var timer='';
var Round_Id=0;
