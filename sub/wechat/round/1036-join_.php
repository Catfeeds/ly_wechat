...
{
	if ($o_round->getScanSum1(0)>=1)
	{
		//说明有抽奖资格
		$o_temp=new WX_User_Activity_Join($o_round->getId(0));
		$o_temp->setRound1(1);
		$o_temp->Save();
		$s_text='恭喜您，洽谈了'.$o_round->getScanSum1(0).'家参展商！<br/><br/>您已进入摇奖池！';
	}else{
		//如果属于KeyAgent，那么可以是5次，搜索KeyAgent资料
		/*$o_user_temp=new WX_User_Info_Temp();
		$o_user_temp->PushWhere(array("&&", "ActivityId", "=", $o_activity->getId(0)));
		$o_user_temp->PushWhere(array("&&", "Phone", "=", $o_user->getPhone(0)));
		$o_user_temp->PushWhere(array("&&", "Type", "=",'Key Agent'));
		if($o_user_temp->getAllCount()>0)
		{
			if ($o_round->getScanSum1(0)>=1)
			{
				$o_temp=new WX_User_Activity_Join($o_round->getId(0));
				...
				$s_text='很遗憾，您洽淡了'.$o_round->getScanSum1(0).'家参展商！<br/><br/>不能参与抽奖哦！';
			}
		}else{
			$s_text='很遗憾，您洽淡了'.$o_round->getScanSum1(0).'家参展商！<br/><br/>不能参与抽奖哦！';
		}*/
